...
- **Automatically replaces deprecated/obsolete packages and references**:
  ...
  - **All absolute path references** (e.g., `C:\...`) are converted to relative paths pointing to `..\Dlls\`:
    - `SLManagedAutomation.dll`, `SLNetTypes.dll`, `SLLoggerUtil.dll`, `Skyline.DataMiner.Storage.Types.dll` → Removed (replaced by the `Skyline.DataMiner.Dev.Automation` NuGet package)
    - Other DLLs (like `SLSRMLibrary.dll`, `DataMinerSolutions.dll`, custom DLLs) → Updated to point to `..\Dlls\` folder
- **Conservative handling of SLSRMLibrary**:
  ...
  - If reference uses an absolute path and no DLL found in repository, path is updated to point to solution-level `Dlls` folder (user must add file manually)
