...
- **Automatically replaces deprecated/obsolete packages and references**:
  ...
  - References to `C:\Skyline DataMiner\` (any subdirectory) for DLLs included in `Skyline.DataMiner.Dev.Automation` (version 10.4.0.22):
...
- **Conservative handling of SLSRMLibrary**:
  ...
  - If reference points to `C:\Skyline DataMiner\` (any subdirectory) and no DLL found in repository, path is updated to point to solution-level `Dlls` folder (user must add file manually)
